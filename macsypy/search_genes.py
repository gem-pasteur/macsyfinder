...
from .gene import ModelGene, CoreGene
from .report import HMMReport

_log = logging.getLogger(__name__)


def worker_cpu(genes_nb: int, cfg: Config) -> tuple[int, int]:
    """
    ...
    :param genes_nb: the number of genes to search
...
def search_genes(genes: list[ModelGene], cfg: Config) -> list[HMMReport]:
    """
    ...
    def search(gene: CoreGene, cpu: int) -> HMMReport:
        """
        ...
        :param cpu: the number of cpu to use (per worker)
        :return: an HMMReport
        """
    ...
    def recover(gene: CoreGene, cfg: Config) -> HMMReport:
        """
