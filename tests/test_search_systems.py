...
class TestSearchSystem(MacsyTest, MacsyEnvManager):
    ...
    def test_disambiguate_cluster_case08(self):
        ...
        self.load_env("env_009")
        try:
            print("\n@@@@@@@@@@@@@@@@@ test_disambiguate_cluster_case08 @@@@@@@@@@@@@@@@@@", file=sys.stderr)
            rep_db = RepliconDB(self.macsy_test_env.cfg)
            rep_info = rep_db['VICH001.B.00001.C001']
            print("@@@ self.macsy_test_env.all_hits", len(self.macsy_test_env.all_hits), file=sys.stderr)
            print("@@@ self.macsy_test_env.system", self.macsy_test_env.system, file=sys.stderr)
            #print("@@@ rep_info", rep_info , file=sys.stderr)
            clusters, multi_syst_genes = build_clusters(self.macsy_test_env.all_hits,
                                                          [self.macsy_test_env.system], rep_info)
            # print("@@@@ clusters",clusters, file=sys.stderr)
            cluster = clusters.clusters[3]
            print("@@@@ cluster", cluster, file=sys.stderr)
            str_ = self.dc_helper(cluster)
            print("=================================", file=sys.stderr)
            print(str_, file=sys.stderr)
            print("=================================", file=sys.stderr)
            print(self.output_control_str('008'), file=sys.stderr)
